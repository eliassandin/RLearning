...
class ConnectFourEnv(gym.Env):
    ...
    def run(self, player1: Player, player2: Player, board: Optional[np.ndarray] = None, render=False) -> ResultType:
        ...
        state_hist = deque([self.__board.copy()], maxlen=None)
        player_turn = []
        player = player1
        act = player.get_next_action(self.__board * cp())
        act_hist = deque([act], maxlen=None)
        player_turn.append(cp())
        ...
        return step_result.res_type, state_hist, act_hist, player_turn
    ...
    def next_state(self, action: int) -> StepResult:
        result = ResultType.NONE

        board_copy = copy.deepcopy(self.__board)*self.__current_player
        if not self.is_valid_action(action):
            raise Exception(
                'Unable to determine a valid move! Maybe invoke at the wrong time?'
            )
